...
**技术栈**: Spring Boot 2.3.6、Spring MVC (spring-boot-starter-web)、MyBatis 2.1.4、Druid 1.2.15、MySQL、Lombok。
...
- 数据源：Druid 连接池，连接 `127.0.0.1:3306/test`，用户名/密码均为 `root`
- MyBatis：mapper XML 文件位于 `classpath:mapper/*.xml`，类型别名包为 `com.example.springbootcoludecode`
- 已注释的配置：Eureka 注册中心、Feign/Hystrix

**数据库**: 使用 Druid 连接池，配置了监控统计过滤器（stat、wall、slf4j）和 prepared statement 缓存。启动时若 MySQL 不可用，Spring 上下文加载会失败。
